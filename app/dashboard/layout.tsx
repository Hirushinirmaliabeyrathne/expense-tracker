...
export default function DashboardLayout({
  ...
  useEffect(() => {
    ...
    if (savedProfile) {
      ...
      // Dispatch an event so other components (like DashboardPage) can react
      window.dispatchEvent(new CustomEvent("profileUpdated", { detail: userProfile }));
    }

    // Listen for profile updates from ProfilePage (if it dispatches this event)
    const handleProfileUpdate = (event: Event) => {
      const customEvent = event as CustomEvent
      if (customEvent.detail) {
        setUser((prev) => ({
        ...
        setProfilePhoto(customEvent.detail.profileImage || "")
        // Also update localStorage here if profileUpdated event means a save happened
  ...
  const handleLogout = () => {
    ...
    // router.push("/auth/login");
    console.log("Logout clicked")
    window.location.href = "/auth/login"; // Simple redirect for demonstration
  }
